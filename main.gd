...
func _ready():
	#gets the width of the board texture
	board_size = $Board.texture.get_width()
	#divide board size by three to get size of individual cell
	cell_size = board_size / 3
	#get coordinates of small panel on right side of window
	player_panel_pos = $PanelPanel.get_position()
	#new game function that is not defined in godot, were creating it ourselves
	new_game()
			...
			if event.position.x < board_size:
				...
				if grid_data[grid_pos.y][grid_pos.x] == 0:
					...
					if check_win() != 0:
						...
						$GameOverMenu.show()
						if winner == 1:
							$GameOverMenu.get_node("ResultLabel").text = "Circle Wins!"
						elif winner == -1:
							$GameOverMenu.get_node("ResultLabel").text = "Cross Wins!"
...
func new_game():
	...
	#create a marker to show starting player's trun
	create_marker(player, player_panel_pos + Vector2i(cell_size / 2, cell_size / 2), true)
	$GameOverMenu.hide()
...
func check_win():
	#add up markers in each node row, column and diagonal
